...
export const handleMoveCards = ({
    ...
    return (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        const moveFromPile = (
            fromPile: CombatAbility[],
        ): { updatedFromPile: CombatAbility[]; movedCards: CombatAbility[] } => {
            ...
            const filteredCardsToMove = cardsToMove.filter((card) => {
                ...
                if (
                    card.removeAfterTurn &&
                    (to === CARD_PILE_TYPES.DISCARD || to === CARD_PILE_TYPES.DEPLETED)
                ) {
                    return false;
                }
                return true;
            ...
            const filteredFromPile = fromPile.filter((card) =>
                cardsToMove.every((movedCard) => movedCard.instanceId !== card.instanceId),
            );
            return {
                updatedFromPile: filteredFromPile,
                movedCards: filteredCardsToMove,
            };
        };
        ...
        if (from === FROM_CARD_PILE_TYPES.ANYWHERE) {
            (["hand", "deck", "discard", "depleted"] as (keyof BattleState)[]).forEach(
                (fromPileName) => {
                    if (fromPileName === to) {
                        return;
                    }
...
export const handleRetrieveDepletedCards = ({
    ...
    return (dispatch: AppDispatch, getState: () => RootState) => {
        const sourceAbilityId = source?.source
            ? (source?.source as CombatAbility)?.instanceId
            : undefined;
        // Prevent eg. Bag From Beyond from pulling itself back out (it can still pull out other Bags From Beyond)
        const battle: BattleState = getState().battle!;
        const eligible = shuffle([
            ...battle.depleted.filter((card: CombatAbility) => card.instanceId !== sourceAbilityId),
        ]);
