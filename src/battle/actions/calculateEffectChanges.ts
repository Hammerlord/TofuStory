 ...
 */
export const calculateEffectChanges = (incomingEffects: CombatEffect[], existingEffects: CombatEffect[]): CombatEffect[] => {
    ...
    incomingEffects.forEach((incomingEffect: CombatEffect) => {
        ...
        if (!count || (count < (incomingEffect.maxApplications || Infinity) && totalStacks < totalStacksCap)) {
            updatedEffects.push(incomingEffect);
            return;
        }

        updatedEffects.forEach((effect: CombatEffect, i) => {
            const { lowestDuration, totalStacks: existingTotalStacks = 0 } = idCountMap[effect.name] || {};
            if (lowestDuration?.id === effect.id) {
                ...
                const maxStacks = Math.min(updatedEffects[i].maxStacks || Infinity, incomingEffect.maxStacks || Infinity);

                updatedEffects[i] = {
                    ...updatedEffects[i],
                    duration: Math.min(effect.maxDuration, (updatedEffects[i].duration || 0) + (incomingEffect.duration || 0)),
                    stacks: Math.min(currentStacks + (incomingEffect.stacks || 1), stacksAllowed, maxStacks),
                    // The last character who applies the DoT gets the applier attribution, eg. for effects like Tauromacis Horn.
