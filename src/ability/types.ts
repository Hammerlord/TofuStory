...
export type EffectEventTrigger = { [key in keyof Action]?: Action[key] } & {
    ...
    // This will push the results of the EffectEventTrigger to the animation queue to play back an animation. Normally it's skipped if the effect event is just a stat change
    pushEventQueue?: boolean;
};
...
export type ActionSummon = {
    ...
    // If a minion name is provided, it will try to tribute an active one with that name. If there are multiple active minions of that name, a random one is chosen.
...
export type Action = {
    damage?: number;
    maxDamage?: number;
    secondaryDamage?: number; // Damage dealt to the secondary targets of an area ability (excluding the primary target)
